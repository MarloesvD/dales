...
!! tendencies is rain water content, droplet number, and total water content, respectively.
...
subroutine initbulkmicrostat
    ...
    if (lnetcdf) then
      ...
      tnextwrite = itimeav+btime
      nsamples = int(itimeav / idtav)
      if (myid==0 .or. lprocblock) then ! If we save domain average, only first processor makes file. If we save per processor each needs to make its own file
...
!> Performs the calculations for the tendencies etc.
  subroutine bulkmicrotend
	...
	if (lprocblock) then
	  ...
	  ! Calculate tendency by sum of Nrp(2:i1,2:j1) & sum all processors. Why not start at 1?
