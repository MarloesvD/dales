    ...
    if(.not.(ltenddec)) return
      ...
      if (lsamptendqt) then
        ...
        do k=1,k1
          ...
          ! Horizontal flux on edges; uwavr = meso + large, so uqtwav is small only
          ...
          ! Vertical flux (including the subgrid flux (diffusion)), excluding block-averaged contributions
          wqtavr = 0.
          if (k == 1) then
            do i=2,i1
            do j=2,j1
